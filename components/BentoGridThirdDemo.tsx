...
export function BentoGridThirdDemo() {
  return (
    <BentoGrid className="max-w-6xl mx-auto mt-40 md:auto-rows-[20rem] px-8 pb-8">
      {items.map((item, i) => (
        <BentoGridItem
          key={i}
          title={item.title}
...
const SkeletonOne = () => {
  ...
  return (
    ...
    >
      <motion.div
        variants={variants}
        className="flex flex-row rounded-full border border-white/[0.2] p-2  items-center space-x-2 bg-black dark:bg-black"
      >
      ...
      <motion.div
        ...
        className="flex flex-row rounded-full border border-white/[0.2] p-2 items-center space-x-2 w-3/4 ml-auto bg-black dark:bg-black"
      >
        <div className="w-full  h-4 rounded-full bg-neutral-900" />
        <div className="h-6 w-6 rounded-full bg-gradient-to-r from-pink-500 to-violet-500 flex-shrink-0" />
      </motion.div>
...
const items = [
  {
    ...
    description: (
      <span className="text-sm">
        Dedicated to building modern,scalable web applications.
      </span>
    ),
    header: <SkeletonOne />,
